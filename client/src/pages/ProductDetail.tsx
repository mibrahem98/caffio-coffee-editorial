...
import TastingReflections from "@/components/TastingReflections";
...
import { coffeeProducts, formatPrice, getCoffeeProduct, getVerifiedTastingNotes, type Lang } from "@/lib/mizanCatalog";
import { responsiveImages } from "@/lib/responsiveImages";

const copy = {
  en: { back: "Back to collection", record: "Product record / working draft", status: "Content status", origin: "Origin", farm: "Farm / producer", process: "Process", altitude: "Altitude", roast: "Roast profile", tasting: "Tasting cues", tastingPending: "Awaiting a verified batch tasting record. No tasting claims are published yet.", brew: "Suggested brew methods", source: "Audit note", sourceBody: "The fields below remain marked as pending until a producer document, batch card, or cupping sheet is attached to the project source registry.", reviewEyebrow: "Customer notes / verified only", reviewTitle: "No reviews published yet.", reviewBody: "Verified customer feedback will appear here once it is collected and approved. Caffio does not publish placeholder ratings or invented testimonials.", reviewAction: "How reviews will be added", related: "Continue through the collection", add: "Add to cart", added: "Added to your coffee cart.", favorite: "Save this coffee", saved: "Saved to your coffees.", demoPrice: "Demo price" },
  ar: { back: "العودة إلى المجموعة", record: "سجل المنتج / مسودة عمل", status: "حالة المحتوى", origin: "المنشأ", farm: "المزرعة / المنتج", process: "المعالجة", altitude: "الارتفاع", roast: "ملف التحميص", tasting: "إيحاءات التذوق", tastingPending: "بانتظار سجل تذوق موثق للدفعة. لا تُنشر إيحاءات تذوق قبل التوثيق.", brew: "طرق التحضير المقترحة", source: "ملاحظة التدقيق", sourceBody: "تبقى الحقول التالية موسومة بأنها قيد التوثيق حتى إرفاق مستند منتج أو بطاقة دفعة أو بطاقة تذوق في سجل المصادر.", reviewEyebrow: "ملاحظات العملاء / موثقة فقط", reviewTitle: "لا توجد مراجعات منشورة بعد.", reviewBody: "ستظهر ملاحظات العملاء الموثقة بعد جمعها واعتمادها. لا تنشر كافيو تقييمات مؤقتة أو شهادات مختلقة.", reviewAction: "كيف ستضاف المراجعات؟", related: "تابع عبر المجموعة", add: "أضف إلى السلة", added: "تمت إضافة القهوة إلى سلتك.", favorite: "احفظ القهوة", saved: "تم حفظها في مفضلاتك.", demoPrice: "السعر التجريبي" },
};
...
export default function ProductDetail() {
  ...
  const verifiedTastingNotes = getVerifiedTastingNotes(product);
  ...
  return <div className="mizan-site detail-site" dir={lang === "ar" ? "rtl" : "ltr"}>
    <PageMeta product={product} lang={lang} />
    ...
    <main className="product-detail-main">
      ...
      <section className="detail-hero"><div className="detail-image"><ResponsiveImage image={responsiveImages[product.imageKey]} alt={`${product.shortName[lang]} coffee`} fetchPriority="high" decoding="async" /><span className="detail-stamp"><span className="mizan-symbol" aria-hidden="true"><i /><b /><em /></span>CAFFIO<br /><b>{product.id.toUpperCase()}</b></span></div><div className="detail-intro"><p className="eyebrow"><span className="eyebrow-dot" /> {product.status[lang]}</p><h1>{product.name[lang]}</h1><p className="detail-profile">{product.profile[lang]}</p><p className="detail-copy">{lang === "ar" ? "صفحة قابلة للتدقيق تجمع ما نعرفه عن المنتج وما ينتظر بطاقة الدفعة. استخدمها كنقطة بداية، لا كبديل عن مستند المصدر." : "A product page that separates what is known from what still needs a batch record. Use it as a starting point, never as a substitute for source documentation."}</p><div className="detail-actions"><button className="button button-gold" onClick={addProduct}>{t.add}<ShoppingBag size={15} /></button><button className={favorite ? "favorite-toggle is-saved" : "favorite-toggle"} onClick={toggleFavorite} aria-pressed={favorite} aria-label={t.favorite}><Heart size={15} fill={favorite ? "currentColor" : "none"} />{favorite ? t.saved : t.favorite}</button><Link className="detail-compare-link" data-testid="compare-from-detail" href={`/compare?a=${product.id}`}>{lang === "ar" ? "قارن هذه القهوة" : "Compare this coffee"}<ArrowUpRight size={14} /></Link><span>{t.demoPrice} / {formatPrice(product.price, lang)} · {product.weight}</span></div></div></section>
      ...
      <section className="detail-notes"><div className="detail-notes-grid"><div><p className="eyebrow"><span className="eyebrow-dot" /> {t.tasting}</p><h2>{lang === "ar" ? "ما الذي نبحث عنه؟" : "What we are looking for."}</h2>{verifiedTastingNotes.length ? <div className="tasting-list" data-testid="verified-tasting-notes">{verifiedTastingNotes.map((note, index) => <span key={index}><i>0{index + 1}</i>{note[lang]}</span>)}</div> : <p className="tasting-pending" data-testid="pending-tasting-notes">{t.tastingPending}</p>}</div><div><p className="eyebrow"><span className="eyebrow-dot" /> {t.brew}</p><div className="brew-methods">{product.brewMethods.map((method) => <span key={method.en}>{method[lang]}<ArrowUpRight size={14} /></span>)}</div></div></div></section>
      <TastingReflections productId={product.id} lang={lang} />
      <section className="related-section"><div className="section-intro"><span className="section-number">MORE</span><div><p className="eyebrow"><span className="eyebrow-dot" /> {t.related}</p><h2>{lang === "ar" ? "قهوة أخرى" : "Another starting point."}</h2></div></div><div className="related-grid">{coffeeProducts.filter((item) => item.id !== product.id).map((item) => <Link key={item.id} href={`/coffee/${item.id}`} className="related-card"><ResponsiveImage image={responsiveImages[item.imageKey]} alt="" loading="lazy" decoding="async" /><div><span>{item.shortName[lang]}</span><strong>{item.profile[lang]}</strong><ArrowUpRight size={15} /></div></Link>)}</div></section>
    </main>
    <footer className="mizan-footer"><span>CAFFIO COFFEE / SPECIALTY ROASTERS</span><span>{lang === "ar" ? "طقس مصمم بعناية" : "A RITUAL, DESIGNED"}</span><span>© 2026 / CONTENT STATUS: DEMO</span></footer>
  </div>;
}
