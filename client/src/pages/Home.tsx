...
import ApprovedReflectionShowcase from "@/components/ApprovedReflectionShowcase";
...
function MizanHome() {
  ...
  const [activeFaqSuggestion, setActiveFaqSuggestion] = useState(-1);
  ...
  const faqSuggestions = faqQuery.trim() ? faqItems.filter((item) => `${item.question} ${item.answer}`.toLocaleLowerCase(lang === "ar" ? "ar" : "en").includes(faqQuery.trim().toLocaleLowerCase(lang === "ar" ? "ar" : "en"))).slice(0, 4) : [];
  const faqSuggestionHasNoMatch = Boolean(faqQuery.trim()) && faqSuggestions.length === 0;
  const selectFaqSuggestion = (item: typeof faqItems[number]) => {
    setFaqQuery(item.question);
    setFaqCategory("all");
    setOpenFaq(item.id);
    setActiveFaqSuggestion(-1);
  };
  ...
  return <div className="mizan-site" dir={direction}>
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      <CoffeeAdvisor lang={lang} />
      <InteractiveBrewGuide lang={lang} />
      ...
      <ApprovedReflectionShowcase lang={lang} />

      <section className="faq-section section-dark" id="faq"><div className="section-intro dark-intro"><span className="section-number">05<span>/</span>06</span><div><p className="eyebrow light"><span className="eyebrow-dot" /> {t.faqEyebrow}</p><h2>{t.faqA}<br /><em>{t.faqB}</em></h2></div></div><div className="faq-discovery" data-testid="faq-discovery"><label className="faq-search"><Search size={16} /><input value={faqQuery} onChange={(event) => { setFaqQuery(event.target.value); setActiveFaqSuggestion(-1); }} onKeyDown={(event) => { if (!faqSuggestions.length) return; if (event.key === "ArrowDown") { event.preventDefault(); setActiveFaqSuggestion((current) => Math.min(current + 1, faqSuggestions.length - 1)); } if (event.key === "ArrowUp") { event.preventDefault(); setActiveFaqSuggestion((current) => Math.max(current - 1, 0)); } if (event.key === "Enter" && activeFaqSuggestion >= 0) { event.preventDefault(); selectFaqSuggestion(faqSuggestions[activeFaqSuggestion]); } if (event.key === "Escape") { setActiveFaqSuggestion(-1); } }} placeholder={faqLabels.search} aria-label={faqLabels.search} role="combobox" aria-autocomplete="list" aria-expanded={Boolean(faqQuery.trim())} aria-controls="faq-suggestions" aria-activedescendant={activeFaqSuggestion >= 0 ? `faq-suggestion-${faqSuggestions[activeFaqSuggestion].id}` : undefined} /></label>{faqQuery.trim() && <ul className="faq-suggestions" id="faq-suggestions" role="listbox" aria-label={lang === "ar" ? "اقتراحات الأسئلة" : "Question suggestions"}>{faqSuggestions.length ? faqSuggestions.map((item, index) => <li id={`faq-suggestion-${item.id}`} key={item.id} role="option" aria-selected={activeFaqSuggestion === index} className={activeFaqSuggestion === index ? "is-active" : ""} onMouseDown={(event) => { event.preventDefault(); selectFaqSuggestion(item); }}>{item.question}</li>) : <li className="is-empty" role="option" aria-disabled="true">{faqLabels.suggestionEmpty}</li>}</ul>}<div className="faq-categories" aria-label={lang === "ar" ? "تصنيفات الأسئلة" : "Question categories"}>{(["all", "choosing", "commerce", "trust", "society"] as const).map((category) => <button type="button" key={category} className={faqCategory === category ? "is-active" : ""} aria-pressed={faqCategory === category} onClick={() => { setFaqCategory(category); setOpenFaq(category === "all" ? "choosing" : category); }}>{faqLabels[category]}</button>)}</div><p className="faq-result-count" role="status">{filteredFaqItems.length} {faqLabels.results}</p></div><div className="faq-list" data-testid="faq-accordion">{filteredFaqItems.length ? filteredFaqItems.map((item, index) => { const isOpen = openFaq === item.id; const panelId = `faq-panel-${item.id}`; return <article className={isOpen ? "faq-item is-open" : "faq-item"} key={item.id}><button className="faq-trigger" type="button" aria-expanded={isOpen} aria-controls={panelId} onClick={() => setOpenFaq(isOpen ? "" : item.id)}><span><small>{String(index + 1).padStart(2, "0")}</small>{item.question}</span><ChevronDown size={17} aria-hidden="true" /></button><div className="faq-answer" id={panelId} aria-hidden={!isOpen}><div><p>{item.answer}</p></div></div></article>; }) : <div className="faq-empty" role="status"><Search size={18} /><p>{faqLabels.empty}</p></div>}</div></section>

      <section className="checkout-preview section-light" aria-labelledby="checkout-preview-title" data-testid="checkout-preview">
        <div className="checkout-preview-head"><div><p className="eyebrow"><span className="eyebrow-dot" /> {t.demoEyebrow}</p><h2 id="checkout-preview-title">{t.demoA}<br /><em>{t.demoB}</em></h2></div><p>{t.demoBody}</p></div>
        <div className="checkout-preview-grid"><article><span>01</span><h3>{t.demoStepOne}</h3><p>{t.demoStepOneBody}</p></article><article><span>02</span><h3>{t.demoStepTwo}</h3><p>{t.demoStepTwoBody}</p></article><article><span>03</span><h3>{t.demoStepThree}</h3><p>{t.demoStepThreeBody}</p></article></div>
        <div className="checkout-preview-actions"><a className="button button-gold" href="/society">{t.demoPrimary}<ArrowUpRight size={16} /></a><a className="text-link" href="/payments">{t.demoSecondary}<ArrowUpRight size={14} /></a></div>
      </section>
