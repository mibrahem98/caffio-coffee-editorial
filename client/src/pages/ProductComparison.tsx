import { ArrowLeft, ArrowUpRight, BookOpen, CheckCircle2, Download, Printer, Scale, Share2 } from "lucide-react";
  ...
  return <div className="mizan-site comparison-site" dir={lang === "ar" ? "rtl" : "ltr"}>
    ...
    <main className="comparison-main">
      ...
      <section className="comparison-workspace" aria-labelledby="comparison-title">
        ...
        {canCompare ? <>
          <div className="comparison-actions"><div><button className="button button-gold" type="button" onClick={shareComparison}><Share2 size={15} />{t.share}</button><button className="comparison-export-button" data-testid="export-comparison-image" type="button" onClick={exportImage}><Download size={15} />{t.exportImage}</button><button className="comparison-export-button" data-testid="print-comparison-pdf" type="button" onClick={printComparison}><Printer size={15} />{t.exportPdf}</button></div><p role="status" aria-live="polite">{shareMessage || exportMessage}</p></div>
